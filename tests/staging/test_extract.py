...
def test_page_is_saved_as_tmp_before_atomic_rename(tmp_path, monkeypatch):
    limit = 10
    total = 5
    staging_dir = str(tmp_path)
    final_path = os.path.join(staging_dir, "page_0000000.json")
    tmp_path_file = final_path + ".tmp"

    real_rename = os.rename
    observed = {}

    def spy_rename(src, dst):
        # Snapshot filesystem state at the exact moment of rename, before it happens:
        # the .tmp file must already exist and hold the write, and the final file must not yet.
        observed["tmp_existed"] = os.path.exists(tmp_path_file)
        observed["final_existed_before_rename"] = os.path.exists(final_path)
        return real_rename(src, dst)

    monkeypatch.setattr("staging.extract.os.rename", spy_rename)
    ...
    assert observed["tmp_existed"] is True
